...
; 	MOV A,H
; 	CPI 24H    ;CY=1 if A < 24H, else CY = 0
; 	JC CHN_MM
...
CHNG_HR_MIN:							;start seconds counter from 0s
	...
	MVI A,00H
INCR_SEC:
	STA CURDT                   ;Insert Accumulator Value (Initially 00H) at CURDT
	CALL UPDAD					;update display of address field and takes data from CURAD and display at address field
	CALL UPDDT                  ;update display of data field and takes data from CURDT and display at data field
	CALL DELAY					;call delay which runs for 1second
	LDA CURDT
	ADI 01H						;increment seconds by 1
	DAA
	CPI 60H						 ;check if seconds reach 60 (A=60H, ZF = 1, else ZF = 0)
	JNZ INCR_SEC                 ;Jump if zero flag is 0
	LHLD CURAD
	MOV A,L
	ADI 01H						;increment minutes by 1
	DAA
	MOV L,A
	CPI 60H 					;check if minutes reach 60 (A=60H, ZF = 1, else ZF = 0)
	JNZ CHNG_HR_MIN
	MVI L,00H
	MOV A,H
	...
	DAA
	MOV H,A
	CPI 13H						;check if hour reach 24
	JNZ CHNG_HR_MIN
	LXI H,0100H
	JMP CHNG_HR_MIN
